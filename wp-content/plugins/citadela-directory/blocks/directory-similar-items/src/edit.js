...
const { TextControl, ToolbarGroup, ToolbarItem, PanelBody, ToggleControl, Icon, Tooltip, SelectControl, RadioControl, ColorPalette, ColorIndicator, BaseControl } = wp.components;
const { apiFetch } = wp;
...
export default class Edit extends Component {
    constructor() {
        super( ...arguments );
        const editPageId = wp.data.select( "core/editor" ) ? wp.data.select( "core/editor" ).getCurrentPostId() : null;
        const itemDetailPageId = parseInt( CitadelaDirectorySettings.specialPages['single-item'] );
        const currentPostType = wp.data.select( "core/editor" ) ? wp.data.select( "core/editor" ).getCurrentPostType() : null;
        this.state = {
            isItemDetail: editPageId === itemDetailPageId || currentPostType === 'citadela-item',
    ...
    render() {
        ...
        let infographicCount = numberOfItems != undefined ? numberOfItems : 10;
        ...
        return (
            <>
                ...
                <InspectorControls key='inspector'>
                    ...
                    >
                        <BaseControl
                            label={ __('Show similar posts from the same:', 'citadela-directory') }
                            className="citadela-toggle-list-control"
                        >
                            <ToggleControl
                                label={__('Item category', 'citadela-directory')}
                                checked={ similarByCategory }
                                onChange={ ( checked ) => setAttributes( { similarByCategory: checked } ) }
                            />
                            <ToggleControl
                                label={__('Item location', 'citadela-directory')}
                                checked={ similarByLocation }
                                onChange={ ( checked ) => setAttributes( { similarByLocation: checked } ) }
                            />
                            <ToggleControl
                                label={__('Item author', 'citadela-directory')}
                                checked={ similarByAuthor }
                                onChange={ ( checked ) => setAttributes( { similarByAuthor: checked } ) }
                            />
                        </BaseControl>

                        <NumberOfItems attributes={ attributes } setAttributes={ setAttributes } />

                        <ToggleControl
                            label={__('Show reference Item Post', 'citadela-directory')}
                            help={__('Selected reference post will be listed before all other similar posts.', 'citadela-directory')}
                            checked={ showReferencePost }
                            onChange={ ( checked ) => setAttributes( { showReferencePost: checked } ) }
                        />
                    ...
                    >
                        <SelectControl
                            ...
                            help={ itemsOrderBy == 'gps' && __('Displayed are nearest items by their GPS coordinates.', 'citadela-directory')}
                            value={ itemsOrderBy }
                            options={ [
                                ...
                                { label: __( 'Distance', 'citadela-directory' ), value: 'gps' },
                            ] }
                            onChange={ ( value ) => { setAttributes( { itemsOrderBy: value } ); }
                            }
                        />

                        { ( itemsOrderBy != 'rand' && itemsOrderBy != 'gps' ) && 
                            <RadioControl
                                selected={ itemsOrder }
                                options={ [
                                ...
                                onChange={ ( value ) => { setAttributes( { itemsOrder: value } ); }
                                }
                            />
                        }
                        { ( itemsOrderBy != 'gps' ) && 
                ...
                >
                    ...
                    { isItemDetail 
                    ?
                        <div class="citadela-directory-posts-selection">
                            <div class="selected-posts">
                                <div class="header">
                                    <div class="title">{__('Reference Item Post', 'citadela-directory' )}</div>
                                    <div class="desc">{__('Already opened Item Post will be used as reference post.', 'citadela-directory' )}</div>
                                </div>
                            </div>
                        </div>
                    :
                        <PostsSelection
                            ...
                            selectedPostsIds={ refPost }
                            onChange={ ( value ) => setAttributes( { refPost: value } ) }
                            titleLabel={ __('Reference Item Post', 'citadela-directory' ) }
                            description={ __('Displayed will be posts similar to selected reference Item Post.', 'citadela-directory' ) }
                            searchLabel={ __('Search for reference item post', 'citadela-directory' ) }
                            singleValue
                            {...this.props}
                        />
                    }
                    
                    { ( Object.keys(refPost).length > 0 || isItemDetail ) &&
                    <>
                        ...
                        <div class="citadela-block-articles">
                            <div class="citadela-block-articles-wrap">
                                {Array.from(Array( infographicCount ), (e, i) => {
                                    return (
...
const NumberOfItems = ({
	...
	const [ inputValue, setInputValue ] = useState( numberOfItems );
	
	return (
		<>
			<BaseControl 
				label={ __( 'Number of items', 'citadela-directory' ) }
                help={ numberOfItems != undefined ? __( 'Empty value to show all similar items.', 'citadela-directory' ) : __( 'Will be listed all available similar items.', 'citadela-directory' ) }
			>
				<input
					...
					onChange={ ( event ) => {
						...
						setAttributes( { numberOfItems: newValue ? parseInt(newValue) : newValue } );
					} }
					value={ inputValue }
					step={ 1 }
                    min={ 1 }
